...
public class PayPalStrategy implements PaymentStrategy {
        ...
        try {
            ...
            if ("approved".equals(executedPayment.getState())) {
                ...
                try {
                    ...
                    if (!spendingSuccess) {
                        log.warn("Failed to record customer SPENDING action to Report Service for customer {}",
                                customerId);
                    }
                } catch (Exception e) {
                ...
                // .notificationContent("Thanh toán thành công cho đơn hàng " +
                ...
                try {
                    ...
                    notificationMicroservice.sendNotification(
                            customer.getId().toString(),
                            "Thanh toán thất bại",
                            "Thanh toán cho booking #" + bookingPayment.getBooking().getId()
    ...
    public BookingPayment refund(UUID bookingId, BookingPayment payment) throws PayPalRESTException {
        ...
        if (!payment.getPaymentMethod().equals(Constants.PaymentMethodEnum.PAYPAL)) {
            ...
            throw new IllegalArgumentException(
                    "Payment method is not PayPal. Cannot refund payment with method: " + payment.getPaymentMethod());
        }
        ...
        // Validate transaction ID format - PayPal Sale IDs should not contain invalid
        // characters
        if (!isValidPayPalTransactionId(transactionId)) {
            log.error("Invalid PayPal transaction ID format: {}. This may not be a valid PayPal Sale ID",
                    transactionId);
            throw new IllegalArgumentException(
     ...
     * - Should NOT contain payment method prefixes like "MOMO_", etc.
    ...
    private boolean isValidPayPalTransactionId(String transactionId) {
        ...
        String[] invalidPrefixes = { "MOMO_", "ZALOPAY_", "BANKING_" };
        ...
        // PayPal transaction IDs should be reasonable length (not too short, not too
        // long)
        ...
        // Allow: alphanumeric, hyphens, underscores, and # (for combined
        // PAY-xxx#SALE-xxx format)
